...
public class SortsTest {
  ...
  private static boolean isSorted(int[] A, int start, int end) {
    ...
    for(int i=start; i<end;i++){ 
      if(i>start&&!(A[i] >= A[i-1])){//will use item i as a refernce point, and compare every item before it to see if there is a value that's greater
          return false;
        } //will do this for each item in array
      /*for(int j=A.asList(array).indexOf(i); j!=0; j--){ 
        if(i!=0&&!(A[i]<=A[j])){//will use item i as a refernce point, and compare every item before it to see if there is a value that's greater
          return false;
          //this will return false if j is ever greater than i. This makes sense. There should be no value that proceeds
          // i that is greater than i
        }
      }kept this work because I thought it was interesting (sue me :P) */
    }
    return true;
  }
