...
public class HashTable<K,V> {
    ...
    Pair previous;  //The previously visited pair when searching for a pair.  Set to null at the beginning of each search.
    ...
    public V get(K key) {
        ...
        Pair current = buckets[keyHash%getCapacity()];
        while(current != null){
    ...
    /** Return the node to which the specified key is mapped, or null if this
     * map contains no mapping for the key.
     * Runtime: average case O(1); worst case O(size) */
    ...
    public V put(K key, V val) {
        int keyHash = Math.abs(key.hashCode());
        int bucket = keyHash%getCapacity();
        if(get(key) == null){
            Pair newPair = new Pair(key, val, buckets[bucket]);
            buckets[bucket] = newPair;
            size++;
            growIfNeeded();
            return null;
        }
    ...
    public V remove(K key) {
        if(getPair(key) != null){
            size --;
            int keyHash = Math.abs(key.hashCode());
            Pair temp = getPair(key);
            if(previous == null){
                buckets[keyHash%getCapacity()] = temp.next;
                return temp.value;
            }
            else{
    ...
    private void growIfNeeded() {
      if((double)size/(double)getCapacity() > .8){
        Pair[] newBuckets = createBucketArray(2 * getCapacity());
        for (int i = 0; i < getCapacity(); i++){
            Pair current = buckets[i];
            while(current != null){
                int currentHash = Math.abs(current.key.hashCode());
                Pair toAdd = new Pair(current.key, current.value, newBuckets[currentHash%newBuckets.length]);
                newBuckets[currentHash%newBuckets.length] = toAdd;
                current = current.next;
            }
        }
        buckets = newBuckets;
